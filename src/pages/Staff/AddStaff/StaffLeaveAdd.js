...
const validationSchema = Yup.object().shape({
  ...
  annualLeave: Yup.string().matches(/^[0-9]+$/, '*Annual Leave Must be numbers').required("*Annual Leave is required!"),
  medicalLeave: Yup.string().matches(/^[0-9]+$/, '*Medical Leave Must be numbers').required("*Medical Leave is required!"),
  otherLeave: Yup.string().matches(/^[0-9]+$/, '*Other Leave Must be numbers').required("*Other Leave is required!"),
 carryForwardLeave: Yup.string().matches(/^[0-9]+$/, '*Carry Forward Leave Must be numbers').required("*Carry Forward Leave is required!"),
  
});
const StaffLeaveAdd=forwardRef(({ formData, setFormData, handleNext }, ref)=>{
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await api.post(`/createUserLeaveCreation/${formData.user_id}`, values, {
          headers: {
      ...
      <section>
        <div className="container" style={{ minHeight: "95vh" }}>
          ...
          <div class="row">
            ...
            />
            {formik.touched.carryForwardLeave && formik.errors.carryForwardLeave && (
                  <div className="error text-danger ">
